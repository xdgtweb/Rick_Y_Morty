<!DOCTYPE html>
<!--
Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
Click nbfs://nbhost/SystemFileSystem/Templates/Other/html.html to edit this template
-->
<html>
    <head>
        <title>Rick y Morty</title>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <link rel="stylesheet" href="./Estilos/Temporada 4.css"/>
        <link rel="stylesheet" href="./Estilos/Contacto y donacion.css"/>
        <link rel="icon" type="image/png" href="Imagenes/Index/rickextrañado.png">
        <sound src="Musica/Index/Space Song.mp3" type="audio/mp3" autoplay="none" loop="infinite"></sound>
    </head>
    <body>
        <header>
            <div class="titulo">
                <label>
                </label>
            </div>
            
        </header>
        
        <div class="navpadre">
        <nav>
            <a href="Temporada1.php"><div class="caja1"><div class="caja2"></div><label>Temporada 1</label></div></a>
            <a href="Temporada2.php"><div class="caja1"><div class="caja2"></div><label>Temporada 2</label></div></a>
            <a href="Temporada3.php"><div class="caja1"><div class="caja2"></div><label>Temporada 3</label></div></a>
            <a href="Temporada4.php"><div class="caja1"><div class="caja2"></div><label>Temporada 4</label></div></a>
            <a href="Temporada5.php"><div class="caja1"><div class="caja2"></div><label>Temporada 5</label></div></a>
            <a href="Temporada6.php"><div class="caja1"><div class="caja2"></div><label>Temporada 6</label></div></a>
        </nav>
        
        <div class="nav2">
            <a href="PROVISIONAL.html"><div class="caja3"><div class="caja4"></div><label>Detalles</label></div></a>
            <a href="PROVISIONAL.html"><div class="caja5"><div class="caja6"></div><label>Teorias</label></div></a>
            <a href="PROVISIONAL.html"><div class="caja7"><div class="caja8"></div><label>Mierdas</label></div></a>
        </div>
        </div>
        
        <main>
            
            <div class="main2"><label>Temporada 4</label></div>
            
            <div class="main3">
                <div class="cajamain1">
                    <a href="IdiomasT4C1.html" >
                        <label>Capitulo 1</label>
                        <div class="previsualizacion1">
                            <div class="playlogo"></div>
                        </div>
                        <div class="tituloEpisodio">
                            <label>Aprovechando el Mortymento: Vive, Muere, Rickpeat</label>
                        </div>
                    </a>
                </div>
                <div class="cajamain2">
                    
                    <a href="IdiomasT4C2.html" >
                        <label>Capitulo 2</label>
                        <div class="previsualizacion2">
                            <div class="playlogo"></div>
                        </div>
                        <div class="tituloEpisodio">
                            <label>El Anciano y el Trono</label>
                        </div>
                    </a>
                </div>
                <div class="cajamain3">
                    
                    <a href="IdiomasT4C3.html" >
                        <label>Capitulo 3</label>
                        <div class="previsualizacion3">
                            <div class="playlogo"></div>
                        </div>
                        <div class="tituloEpisodio">
                            <label>Una Pandilla para el Patético Morty</label>
                        </div>
                    </a>
                </div>
                <div class="cajamain4">
                    
                    <a href="IdiomasT4C4.html" >
                        <label>Capitulo 4</label>
                        <div class="previsualizacion4">
                            <div class="playlogo"></div>
                        </div>
                        <div class="tituloEpisodio">
                            <label>La Ley y el Dragorden</label>
                        </div>
                    </a>
                </div>
                <div class="cajamain5">
                    
                    <a href="IdiomasT4C5.html" >
                        <label>Capitulo 5</label>
                        <div class="previsualizacion5">
                            <div class="playlogo"></div>
                        </div>
                        <div class="tituloEpisodio">
                            <label>Rattlestar Rickláctica</label>
                        </div>
                    </a>
                </div>
                <div class="cajamain6">
                    
                    <a href="IdiomasT4C6.html" >
                        <label>Capitulo 6</label>
                        <div class="previsualizacion6">
                            <div class="playlogo"></div>
                        </div>
                        <div class="tituloEpisodio">
                            <label>Un Tren de Antologías</label>
                        </div>
                    </a>
                </div>
                <div class="cajamain7">
                    
                    <a href="IdiomasT4C7.html" >
                        <label>Capitulo 7</label>
                        <div class="previsualizacion7">
                            <div class="playlogo"></div>
                        </div>
                        <div class="tituloEpisodio">
                            <label>Alienígenas</label>
                        </div>
                    </a>
                </div>
                <div class="cajamain8">
                    
                    <a href="IdiomasT4C8.html" >
                        <label>Capitulo 8</label>
                        <div class="previsualizacion8">
                            <div class="playlogo"></div>
                        </div>
                        <div class="tituloEpisodio">
                            <label>El Episodio del Tanque de Ácido</label>
                        </div>
                    </a>
                </div>
                <div class="cajamain9">
                    
                    <a href="IdiomasT4C9.html" >
                        <label>Capitulo 9</label>
                        <div class="previsualizacion9">
                            <div class="playlogo"></div>
                        </div>
                        <div class="tituloEpisodio">
                            <label>Childrick of Morta</label>
                        </div>
                    </a>
                </div>
                <div class="cajamain10">
                    
                    <a href="IdiomasT4C10.html" >
                        <label>Capitulo 10</label>
                        <div class="previsualizacion10">
                            <div class="playlogo"></div>
                        </div>
                        <div class="tituloEpisodio">
                            <label>El Rickgreso del Jerri</label>
                        </div>
                    </a>
                </div>
                
            </div>
            
        </main>
        
        <footer>
            <div class="footer1">
                <label>ESTA PAGINA WEB SE HA HECHO SIN ANIMO DE LUCRO Y COMO PARODIA DE LA WEB RICK Y MORTY OFICIAL</label>
            </div>
            <div class="footer2">
                <label>Politica de prickvacidad</label>
                <div class="cajafooter1">
                    <label>Opiniones de ricknuncios
                    </label>
                    <div class="logoanucio"></div>
                </div>
                <label>Configuración de ricookies</label>
                <label>Términos de rickuso</label>
                <label>Controles rickrentales</label>
                <label>Ricktítulos</label>
                <label>Ricktáctanos</label>
                <label>Sala de rickprensa</label>
                <label>Rickyuda</label>
                <label>Mapa del ricksitio</label>
            </div>
            <div class="footer3">
                <label>
                    TM & © 2022 Fawful Inc. Todos los derechos reservados.
                </label>
            </div>
        </footer>
        
        <div class="menu">
            <a href="Index.html">
                    Menú
            </a>
        </div>
        
    </body>
    
    <div class="contacto">

        <a href="https://t.me/XDavidGarciaTorres">
            <img src="Imagenes/Contacto y Donacion/telegram.png" width="35" height="35" alt="telegram"/>
        </a>
        <a href="https://instagram.com/gracovitz?igshid=YmMyMTA2M2Y=">
            <img src="Imagenes/Contacto y Donacion/instagram.png" width="35" height="35" alt="instagram"/>
        </a>
        <a href="https://www.paypal.com/paypalme/xdgtweb">
            <img src="Imagenes/Contacto y Donacion/paypal.png" width="35" height="35" alt="donacion"/>
        </a>

    </div>
    
        <?php
    $archivo = "./Contadores de visitas/ContadorTemporada4.txt";
    $contador = intval(trim(file_get_contents($archivo)));

    $archivo = fopen($archivo, "w");
    fwrite($archivo, $contador + 1);

//    $archivo = fopen($archivo, "r");
    echo '<div style="position:fixed;'
    . 'font-family: ordenador;'
    . 'letter-spacing: 1.3px;'
    . 'font-size: 15px;'
    . 'bottom: 10px;'
    . 'z-index:9;'
    . 'right: 10px;'
    . 'border: 1px #FF8000 solid;'
    . 'box-shadow: 0px 0px 10px #FF8000, inset 0px 0px 20px #FF8000;'
    . 'background: #3B240B;'
    . 'padding: 5px;'
    . '-webkit-text-stroke: 0.7px #b4df24;'
    . 'color: #01b5cc;'
    . 'border-radius: 7px;">'
    . 'Visitas: ' . $contador . '</div>';

    fclose($archivo);
    ?>
    
</html>
